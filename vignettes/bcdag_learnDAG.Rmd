...
<!-- Obs: HO MODIFICATO S = 500 e BURN = 100 PER VELOCIZZARE LA COMPILAZIONE; ALLA FINE RICORDATI DI RIMETTERLI A POSTO -->

<!-- NON SONO RIUSCITO A INCLUDERE LE REF, HO LASCIATO INDICATO DOVE VAN MESSE; VEDI TU SE VUOI CIMENTARTI, SE NO LASCIAMO STARE -->

<!-- NON MI PRENDE né emph né textit -->
...
out <- learn_DAG(S = 500, burn = 100, data = X,
                 a, U, w, 
...
depends on the choice of boolean arguments `save.memory` and `collapse`.
...
```{r echo = FALSE, include=FALSE}
collapsed_out <- learn_DAG(S = 500, burn = 100, data = X,
                 a, U, w, 
...
```{r echo = FALSE, include=FALSE}
compressed_out <- learn_DAG(S = 500, burn = 100, data = X,
                 a, U, w, 
...
```{r echo = FALSE, include=FALSE}
comprcoll_out <- learn_DAG(S = 500, burn = 100, data = X,
                 a, U, w, 
...
```{r results='hide'}
# No approximation
time_nofast <- system.time(out_nofast <- learn_DAG(S = 500, burn = 100, data = X, 
                      a, U, w, 
                      fast = FALSE, save.memory = FALSE, collapse = FALSE))
# Approximation
time_fast <- system.time(out_fast <- learn_DAG(S = 500, burn = 100, data = X, 
                      a, U, w, 
...
*Bayesian Analysis*, In press. doi: https://doi.org/10.1214/21-BA1260.
...
doi: https://doi.org/10.1198/10618600152627924.
