...
<!-- HO ABBASSATO IL NUMERO DI ITERAZIONI A 500 (CON BURN 100); RICORDIAMOCI DI RIMETTERLO A POSTO ALLA FINE -->
...
<!-- For each possible edge in the graph, $u\rightarrow v$, $u \ne v$, the corresponding posterior probability of inclusion is estimated as the proportion of DAGs in the MCMC output containing the directed edge $u\rightarrow v$; see also Castelletti \& Mascaro (2021). -->
...
<!-- [QUESTO VALE NEL CASO SINGLE-NODE INTERVENTION, CORRETTO? LA FUNZIONE SI APPLICA ANCHE AL CASO JOINT PERO'] -->
...
```{r}
effects_out <- out |>
  get_causaleffect(targets = c(4,5), response = 1)
head(effects_out)
```
...
* Castelletti, F, Mascaro, A (2021). "Structural learning and estimation of joint causal effects among network-dependent variables". *Statistical Methods & Applications*, 30(5), 1289-1314. doi: https://doi.org/10.1007/s10260-021-00579-1.

* Nandy, P, Maathuis, MH., Richardson, TS (2017). "Estimating the effect of joint interventions from observational data in sparse high-dimensional settings". *The Annals of Statistics*, 45(2), 647-674. doi: https://doi.org/10.1214/16-AOS1462.
