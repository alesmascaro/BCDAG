...
vignette: >
  %\VignetteIndexEntry{Random generation from Gaussian DAG models}
...
<!-- NON MI PRENDE né emph né textit, alcune parole rimangono quindi "nascoste" -->
...
<!-- \begin{equation} -->
<!-- \boldsymbol{x} = (\boldsymbol I-\boldsymbol L)\boldsymbol x + \boldsymbol \epsilon, \quad \boldsymbol \epsilon \sim \mathcal{N}_q(\boldsymbol 0, \boldsymbol D), -->
<!-- \end{equation} -->
<!-- CREDO SIA CORRETTO IL SEGUENTE (CON IL TRASPOSTO): -->
\begin{equation}
...
The DAG-Wishart distribution (Cao et al., 2019) provides a conjugate prior for $(\boldsymbol L, \boldsymbol D)$.
Moreover, a *compatible* DAG-Wishart distribution, guaranteeing score equivalence for Markov equivalent DAGs, was introduced by Peluso \& Consonni (2020); see the original paper for full details.
...
$\boldsymbol{U}$ is the rate parameter (a $(q,q)$ s.p.d. matrix) and $\boldsymbol{a}^{\mathcal {D}}_{c}$ (a $(q,1)$ vector) is the shape parameter of the DAG-Wishart distribution. Moreover, in its compatible version, elements of the vector parameter $\boldsymbol{a}^{\mathcal {D}}_{c}$ are uniquely determined from a single *common* shape parameter $a>q-1$.
...
Data generation from a Gaussian DAG model is finally straightforward.
...
for Gaussian directed acyclic graph models.” *arXiv pre-print*. URL https://arxiv.org/abs/1109.4371.
...
doi: https://doi.org/10.1214/18-AOS1689.
...
doi: https://doi.org/10.1007/s10260-021-00579-1.
...
Gaussian DAGs.” *Electronic Journal of Statistics*, 14(2), 4110–4132. doi: https://doi.org/10.1214/20-EJS1768.
